...
policy_df_0 <- list_all$State.Characteristics %>%
  select(state, state_abbreviation, state_fips_code,
         ...
         left_join(list_all$State.Of.Emergency %>%
                     select(state, state_of_emergency_declared = state_of_emergency_issued,),by = 'state') %>%
         left_join(list_all$Masks %>%
              select(state, begin_mask_mandate = public_face_mask_mandate, end_face_mask_mandate),by = 'state') %>%
  mutate(begin_mask_mandate = as.Date(begin_mask_mandate, origin = '1899-12-30'),end_face_mask_mandate = as.Date(end_face_mask_mandate, origin = '1899-12-30'))%>%
...
#Assess whether to use mean or median change in transit
...
#Calculate the mean county-population-weigthed value of transit changes for each state in the first 365 days after emergency declaration
...
Given that the distribution of relative transit change had a left skew, our team decided to use the median value for each state within the 365 day window as a better measure of central tenancy. 
...
ggplot(data = final_df) +
  ...
  labs(title = 'State-Level One Year Median Transit Change vs. 
       Cases Per 100K People at One Year After SOE', x='Median Daily Relative % Transit Change', y='Cases Per 100K People One Year After SOE') 
```
...
```{r EDA population density}
density_df=final_df[c('cases_per_100k_at_365d','median_transit_change','pop_pct_age_0_24','population_density')]
pairs(density_df,lower.panel = panel.cor,upper.panel = panel.smooth)
...
qplot(model_2b$fitted, model_2b$residuals,
         ...
         labs(title = "Plot of residuals vs fitted values",
              ...
              y = "residual")
```
After looking at age and density, we were interested in whether policy changes . 


Here’s the EDA that justifies testing these with our model



```{r difference unemployment benefit days}
...
model_1 <- lm(cases_per_100k_at_365d ~ diff_in_days, data = m1)
qplot(model_1$residuals,
               geom = "histogram", bins = 20) +
         labs(title = "Histogram of residuals",
              x = "residual")
...
ggplot(data = final_df) +
  geom_point(aes(x = unemployment_benefits_days,
                 y = cases_per_100k_at_365d)) +
  geom_smooth(aes(x =unemployment_benefits_days,
                 y = cases_per_100k_at_365d),
            colour = 'red',
            method = "lm", se = FALSE)
...
```{r difference mask mandate days}
...
ggplot(data = final_df) +
  geom_point(aes(x = mask_mandate_days,
                 y = cases_per_100k_at_365d)) +
  geom_smooth(aes(x = mask_mandate_days,
                 y = cases_per_100k_at_365d),
            colour = 'red',
            method = "lm", se = FALSE)
...
```
We selected the following policy-related features from the COVID-19 US State Policy Database (www.tinyurl.com/statepolicies) that we thought related to COVID cases including, state of emergency declared, beginning and end of mask mandate, beginning and end of stay-at-home order, beginning and end of business closure, beginning and end of travel quarantine mandate, beginning and end of increased unemployment benefits, and increased unemployment insurance amount. All the policy-related features were recorded as dates, except for the increased unemployment insurance amount that was recorded as integers. 

However, what we were interested in were the total days of the selected policies being declared within one year after the state of emergency was announced for each state. Hence, we conducted the following calculations to obtain the total days:

1. If a policy had no beginning and end dates, the total days were assigned as zero.
2. If a policy had beginning but not end dates, which meant the policy lasted to the day we pulled the data from the database, the total days were calculated by the date of state emergency declared + 364 days - the date of the beginning of the policy.
3. If a policy had both beginning and end dates, the total days were calculated by the difference in days of the two dates.


```{r difference stay at home days}
...
#m1$diff_in_days[m1$diff_in_days >300] <- NA
...
# m1$diff_in_days[m1$diff_in_days > 365] <- 365
final_df$business_close_open_days = m1$diff_in_days



```

Once the features of our interest were calculated, we conducted EDA on the policy-features and COVID cases per 100k population using a scatter matrix plot. 

```{r policy days vs cases per capita}
...
pairs(m1,lower.panel = panel.cor,upper.panel = panel.smooth)



```
From the scatter matrix plot, we have observed that all the values of the correlations between policy-related features and COVID cases per 100k population were between -0.2 to -0.5. The values of the correlations indicated that the policy-related features were weakly and negatively correlated to the COVID cases per 100k population. The directionality of the policy-related features, which were declared to mitigate COVID spread, appeared to be expected. Statistical significance of the features were tested in the following model building and selection sections.

```{r lm anova}


model_2a <- lm(cases_per_100k_at_365d ~ median_transit_change  , data =final_df)
# Significant
anova(model_2a, model_1, test = "F")
model_2b <- lm(cases_per_100k_at_365d ~ unemployment_benefits_days , data = final_df)
cat("\n-------Model results-------\n")
summary(model_2b)
cat("\n")
# Significant
...
To further explore the whether including one or more government policies provides incremental information to the model, we tested the different configurations following a greedy algorithm, starting with days of mask mandate the state government enacted(mask_mandate_day). We assume mask mandate should lead to fewer COVID cases.  Meanwhile, the correlation between days of mask mandate and median transit change is relatively low at -0.34, though the correlation between mask mandate and population density is higher at 0.46. We specified our model as:


$$ 
\text{ Model  Iteration  1  }:\   
\begin{equation}
  \text{cases_per_100k_at_365d}=\beta_0 + \beta_1 \text{median_transit_change} + \beta_2 \text{pop_pct_age_0_24} + \beta_3 \text{population_density} + \beta_4    \text{mask_mandate_days} + \epsilon
\end{equation}
$$
This model results show a marginal increase in Adjusted R-squared,  0.49 compared to 0.48 for the base model. However, the F statistics decreased from 16.02 for the base model to 12.62 for the Trail Model 1. The coefficient for the new covariate (mask_mandate_days) is negative as expected but p value is 0.2 which indicates the new covariate is statistically insignificant. We suspected that... is insignificanct is because.....   With decreased F statsitics and the insignificance of coefficient, we decided not to include the length of enforcing mask mandate in our model and change to look at unemployment benefit related policies. 

```{r model2 adding mask_mandate_days to base model}

model_base=cases_per_100k_at_365d ~ median_transit_change + pop_pct_age_0_24 + population_density 
model_II=cases_per_100k_at_365d ~ median_transit_change + pop_pct_age_0_24 + population_density + mask_mandate_days

lm.II=lm(model_II,data=final_df)
summary(lm.II)
resid(lm.II)
anova(lm.II, lm.I, test = "F")

# mdl_name<-c('model_Base','model_II')
# 
# A_Rsquared<-c(summary(lm.I)$adj.r.squared,summary(lm.II)$adj.r.squared,summary(lm.III)$adj.r.squared,summary(lm.IV)$adj.r.squared,summary(lm.V)$adj.r.squared,summary(lm.VI)$adj.r.squared)
# 
# df<-data.frame(mdl_name,A_Rsquared)
#shapiro.test(lm.II$residuals)
#bptest(lm.II)
```

COVID pandemic caused massive job losses while people in poor economic conditions might be more likely to be exposed to COVID risks. Thus, the next step, we assessed the effects of increased unemployment benefit each state government provided on COVID cases in each state. We included two covariates on this front: 1. days of increased unemployment benefit by state government(unemployment_benefits_days); 2. the amount of the increased unemployment insurance(increased_weekly_unemployment_insurance_amt_thru_jul31). We specified two models as:

$$ 
\text{Model  Iteration  2  }:\ 
\begin{equation}
  \text{cases_per_100k_at_365d}=\beta_0 + \beta_1 \text{median_transit_change} + \beta_2 \text{pop_pct_age_0_24} + \beta_3 \text{population_density} + \beta_4    \text{unemployment_benefits_days} + \epsilon
\end{equation}
$$

```{r model3 adding unemployment benefit days to base model}
model_base=cases_per_100k_at_365d ~ median_transit_change + pop_pct_age_0_24 + population_density 
model_III=cases_per_100k_at_365d ~ median_transit_change + pop_pct_age_0_24 +population_density + unemployment_benefits_days
lm.III=lm(model_III,data=final_df)
summary(lm.III)
anova(lm.III, lm.I, test = "F")
resid(lm.III)
#shapiro.test(lm.III$residuals)
#bptest(lm.III)
```

$$
\text{Model Iteration 3  }:\ 
\begin{equation}
  \text{cases_per_100k_at_365d}=\beta_0 + \beta_1 \text{median_transit_change} + \beta_2 \text{pop_pct_age_0_24} + \beta_3 \text{population_density} + \beta_4    \text{increased_weekly_unemployment_insurance_amt_thru_jul31} + \epsilon
\end{equation}
$$
```{r model 4 adding insurance increase to base model}
model_base=cases_per_100k_at_365d ~ median_transit_change + pop_pct_age_0_24 + population_density 
model_IV=cases_per_100k_at_365d ~ median_transit_change + pop_pct_age_0_24 + population_density + increased_weekly_unemployment_insurance_amt_thru_jul31
lm.IV=lm(model_IV,data=final_df)
summary(lm.IV)
anova(lm.IV, lm.I, test = "F")
resid(lm.IV)
#shapiro.test(lm.IV$residuals)
#bptest(lm.IV)
```
The test statistics of the two models tells that the effects of adding either the days of enacting increased unemployment benefit and the amount of increased unemployment benefit is not substantial. Similar to the mask mandate, although we saw a marginal increase in Adjusted R-squared but the F-statistics decreased from that of base model. In addition, the p values of the coefficients of days of enacting increased unemployment benefit and amount of unemployment insurance increase are 0.16 and 0.10 respectively. Using a 5% threshold value as most academic research, both coefficients are insignificant. During EDA ...





In the last step, we included three additional coveriates: business closures(business_close_open_days), travel quarantines(travel_quarantine_mandate_days) and stay at home orders(stay_at_home_days). These three covariates seems conceptually to be collinearity correlated with transit change so we left them to the last to test.
$$
\text{Model Iteration 4  }:\ 
\begin{equation}
  \text{cases_per_100k_at_365d}=\beta_0 + \beta_1 \text{median_transit_change} + \beta_2 \text{pop_pct_age_0_24} + \beta_3 \text{population_density} + \beta_4    \text{business_close_open_days} + \epsilon
\end{equation}
$$
```{r model5 adding business close open days to base model}
model_base=cases_per_100k_at_365d ~ median_transit_change + pop_pct_age_0_24 + population_density 
model_V=cases_per_100k_at_365d ~ median_transit_change + pop_pct_age_0_24 + population_density + business_close_open_days
lm.V=lm(model_V,data=final_df) 
summary(lm.V)
anova(lm.V, lm.I, test = "F")
resid(lm.V)
#shapiro.test(lm.V$residuals)
#bptest(lm.V) 
```

$$
\text{Model Iteration 5  }:\ 
\begin{equation}
  \text{cases_per_100k_at_365d}=\beta_0 + \beta_1 \text{median_transit_change} + \beta_2 \text{pop_pct_age_0_24} + \beta_3 \text{population_density} + \beta_4    \text{travel_quarantine_mandate_days} + \epsilon
\end{equation}
$$


```{r model6 adding travel_quarantine_mandate_days to base model}
model_base=cases_per_100k_at_365d ~ median_transit_change + pop_pct_age_0_24 + population_density 
model_VI=cases_per_100k_at_365d ~ median_transit_change + pop_pct_age_0_24 + population_density + travel_quarantine_mandate_days


lm.VI=lm(model_VI,data=final_df) 
summary(lm.VI)
anova(lm.VI, lm.I, test = "F")
resid(lm.VI)
#shapiro.test(lm.VI$residuals)
#bptest(lm.VI) 
```

$$
\text{Model Iteration 6  }:\ 
\begin{equation}
  \text{cases_per_100k_at_365d}=\beta_0 + \beta_1 \text{median_transit_change} + \beta_2 \text{pop_pct_age_0_24} + \beta_3 \text{population_density} + \beta_4    \text{stay_at_home_days} + \epsilon
\end{equation}
$$


Through the modeling process, we found adding the policy variables were unlikely to provide incremental information to the model. On one hand, the outcomes of policies might already be partially captured by the change of the transit; on the other hand, the population density might be one of the important reason for a state government to decide on whether enforce certain policies and or how long to enact the policies. 


```{r model7 adding stay at home days to base model}

model_base=cases_per_100k_at_365d ~ median_transit_change + pop_pct_age_0_24 + population_density 
model_VII=cases_per_100k_at_365d ~ median_transit_change + pop_pct_age_0_24 + population_density + stay_at_home_days

lm.VII=lm(model_VII,data=final_df) 
summary(lm.VII)
anova(lm.VII, lm.I, test = "F")
resid(lm.VII)
#shapiro.test(lm.VII$residuals)
#bptest(lm.VII) 
```
