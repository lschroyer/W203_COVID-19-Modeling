...
first_90 <- US_cases_and_mobility %>% 
  ...
  filter(as.Date(date) >= as.Date(state_of_emergency_declared) & 
           as.Date(date) <= as.Date(state_of_emergency_declared) + 355)
...
```{r age EDA, echo=FALSE, message=FALSE}
census_df<-final_df[c('cum_cases_at_90d','Percent.Sex.And.Age.Total.Population.15.To.19.Years','Percent.Sex.And.Age.Total.Population.20.To.24.Years','Percent.Sex.And.Age.Total.Population.25.To.34.Years','Percent.Sex.And.Age.Total.Population.35.To.44.Years','Percent.Sex.And.Age.Total.Population.45.To.54.Years','Percent.Sex.And.Age.Total.Population.55.To.59.Years','Percent.Sex.And.Age.Total.Population.60.To.64.Years','Percent.Sex.And.Age.Total.Population.65.To.74.Years')]

census_df2<-census_df

names(census_df) <- names(census_df) %>% gsub(pattern = "Percent.Sex.And.Age.Total.Population.", replacement = "Pct. ") %>%  gsub(pattern = ".To.", replacement = "- \n") %>%  gsub(pattern = ".Years", replacement = " Yrs") %>%  gsub(pattern = 'cum_cases_at_90d', replacement = " Cases per \n 100k")
...
```{r case v.s. age group, echo=FALSE, message=FALSE}
...
myplot(census_df2,census_df2$Percent.Sex.And.Age.Total.Population.25.To.34.Years,census_df2$cum_cases_at_90d)

myplot_log(census_df2,census_df2$Percent.Sex.And.Age.Total.Population.25.To.34.Years,census_df2$cum_cases_at_90d)
```
