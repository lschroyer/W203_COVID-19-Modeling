...
| Population Density (People/sq. mi.) | 2, 3  | US Census Data |
...
```{r read in NYT covid data, echo=TRUE, warning=FALSE, message=FALSE}
...
```{r read in policy data script, echo=TRUE, message=FALSE, warning=FALSE}
...
*Note: the rest of the R code used to read in the remaining observations/datasets has been suppressed for report readability. From this point onward, we only print code that is necessary for the report narrative. Please refer do this project's [GitHub repository](https://github.com/lschroyer/W203_Lab2_ChangMitchellQianSchroyer.git) for the full R-markdown file*

```{r edit policy data, echo=FALSE, message=FALSE, warning=FALSE}
...
The first step of our exploratory data analysis (EDA) was to determine which mobility metric to use from the Google mobility dataset in order to answer our intial secondary research question, which was: “How does mobility impact the spread of COVID-19?” We began by exploring the correlations that existed in between the Google observations between Transit, Retail, Grocery, Workplace, Residential, and Parks.
...
cat("Park Mobility and COVID CASES per 100K People Regression for EDA")
...
```{r Model 1: Transit Mobility beginings, echo=FALSE, message=FALSE, fig.align='center', fig.width=4, fig.height=3}
#print new line
cat(" \n \n \n \n")
...
ggplot(data = final_df) +
  ...
  labs(title =
         'Percentage Change in Median Transit
Mobility vs. Cases Per 100K People',
...
- the calcualted robust standard errors for the OLS
- Shapiro-Wilk normality of residuals test

Throughout the report, we continued to run these tests (in additon to some other graphics and statistics). The result summary for these include:

```{r model 1, echo=FALSE, message=FALSE,fig.width=4, fig.height=3, fig.align='center'}
...
qplot(model_1_final$fitted, model_1_final$residuals,
         ...
         geom_smooth(aes(x = model_1_final$fitted,
            ...
            colour = 'blue',
            method = "loess", se = TRUE) +
         labs(title = "Model 1 Plot of Residuals vs Fitted Values",
              x = "Fitted Values",
              y = "Residual") +
  theme(axis.text=element_text(size=8),
        axis.title=element_text(size=8),
...
```{r age EDA less than 24, echo=TRUE, message=FALSE, fig.width= 6, fig.height=3.5}
...
pairs(census_df,
      ...
      labels = c("Cases", "Age 60-64%", "Age 65-74%", 
                 "Age 75-86%", "Age 85+ %"),
      main = "Case vs Age (Contin. 2) Bins Distribution and Scatter Plots")
```
...
```{r incremental variables 1, echo=FALSE, fig.align="center", message=FALSE, fig.width=4, fig.height=3}
#print new line
cat(" \n \n \n \n")
...
```{r incremental variables 2, echo=FALSE, message=FALSE, fig.align="center", fig.width=4, fig.height=3}
#print new line
cat(" \n ")
...
pairs(m2,
      lower.panel = panel.cor, upper.panel = panel.smooth, 
      diag.panel = panel.hist, cex.labels=1, gap = 0.5,
      labels = c("cases", "businesss closed days", 
                 "travel quarantine", "stay at home"),
      main = 
...
```{r fig.cap = "Correlation Matrix: State Policies vs. Cases", echo=FALSE, message=FALSE, warning=FALSE, fig.width=12, fig.height=6, out.width="90%"}
corrplot(mat1, method = "color", 
         tl.col = 'black', is.corr=FALSE, 
         # title = 'Correlation Matrix: State Policies vs. Cases',
         addCoefasPercent = TRUE,  addCoef.col = "white", 
...
```{r mask mandate substitution concept, echo=FALSE}
...
model_2_final <- lm(cases_per_100k_at_365d ~ median_transit_change + 
...
qplot(model_2_final$fitted, model_2_final$residuals,
         ...
         geom_smooth(aes(x = model_2_final$fitted,
            ...
            colour = 'blue',
            method = "loess", se = TRUE) +
         labs(title = "Model 2 Plot of Residuals vs Fitted Values",
              x = "fitted value",
              y = "residual") +
  theme(axis.text=element_text(size=8),
        axis.title=element_text(size=8),
...
z <- data.frame(
  ...
  dplyr::rename(
             ...
             'Transit Mobility Change' = final_df.median_transit_change,
...
qplot(model_3_final$fitted, model_3_final$residuals,
         ...
         geom_smooth(aes(x = model_3_final$fitted,
            ...
            colour = 'blue',
            method = "loess", se = TRUE) +
         labs(title = "Model 3 Plot of residuals vs fitted values",
              x = "fitted value",
              y = "residual") +
  theme(axis.text=element_text(size=8),
        axis.title=element_text(size=8),
...
#           font.size = "small",
#           single.row = TRUE,
#           out = "pwd/fit_lm.txt") #output text file - save as png, 
#                                   then reload
...
Perfect Colinearity means no independent variable can be expressed as a linear function of any other independent variables. It is clear there is no perfect colinearity since all feature paramaters had an estimation (i.e. the OLS regression function in R automatically drops `n-1` features in the instance where there are `n` perfectly colinear features).
...
```{r base model, echo=FALSE, message=FALSE, fig.align='center', fig.width=4, fig.height=3}
#-------Model Residual Plot-------
m<-mean(model_2_final$residuals)
std<-sqrt(var(model_2_final$residuals))
hist(model_2_final$residuals, freq=F, breaks=12, xlab='Model 2 residuals',
...
The regression analysis showed that the cases per 100,000 people ≈ -10,160 + 82 * percent change in transit mobility levels + 628 * percent of the population under 25 years of age + 5 * population density. The regression coefficients suggested that a 1% change in mobility was associated with approximately 80 case counts increase in the first year, a 1% change in the population under age 25 was associated with 628 cases increase per 100,000 people, and a 1 unit change in the population density was associated with 5 cases increase per 100,000 people when the other two features were kept constant. It would seem that a change in the percentage of the population ages 0-24 is associated with the the greatest change in COVID cases.
